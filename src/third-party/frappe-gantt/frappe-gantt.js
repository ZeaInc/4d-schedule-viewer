var Gantt = (function () {
  ...
  function createSVG(tag, attrs) {
    ...
    for (let attr in attrs) {
      if (attr === "append_to") {
        ...
        for (let st in attrs.style) elem.style[st] = attrs.style[st];
      } else {
  ...
  class Bar {
    ...
    prepare_values() {
      // this.task.invalid = this.task.invalid;
    ...
    bind() {
      ...
      this.task.on("progressChanged", (progress) => {
    ...
    setup_click_event() {
      ...
      $.on(this.group, "click", (e) => {
        ...
        this.show_popup([e.offsetX, e.offsetY]);
      });
    }
  ...
  class Popup {
    ...
    make() {
      ...
      this.content = this.parent.querySelector(".content");
    ...
    show(options) {
      ...
      if (this.custom_html) {
        ...
        this.parent.style.width = this.parent.clientWidth + 5 + "px";
      }
      ...
      if (options.position === "left") {
        ...
        this.parent.style.left = options.position[0] - 10 + "px";
        this.parent.style.top =
          position_meta.y + position_meta.height + 3 + "px";
  ...
  class Gantt {
    constructor(wrapper, schedule, options) {
      ...
      this.schedule.on("tasksAdded", (rootLevelTasks) => {
        const ganttTasks = [];
        ...
        const collectTasks = (task) => {
          ...
          // }
          for (let childTask of task.childTasks) collectTasks(childTask);
        };

        for (let childTask of rootLevelTasks) {
          collectTasks(childTask);
        }

        this.setup_tasks(ganttTasks);
        this.change_view_mode();
      });

      this.schedule.on("currentDateChanged", (currentDate) => {
        this.set_current_date(currentDate, false);
      });
    ...
    set_current_date(current_date, trigger_event = true) {
      ...
      const x =
        (date_utils.diff(this.current_date, this.gantt_start, "hour") /
          this.options.step) *
        this.options.column_width;
      this.timeSlider.setAttribute("x", x);
    ...
    make_grid_highlights() {
      ...
      const width = this.view_is("Day") ? this.options.column_width : 10;
      const height =
        (this.options.bar_height + this.options.padding) * this.tasks.length +
        this.options.header_height +
        this.options.padding / 2;

      this.timeSlider = createSVG("rect", {
        x,
        y,
        width,
        height,
        class: "today-highlight",
        append_to: this.layers.overlay,
      });
      // }
    }
    ...
    bind_bar_events() {
      ...
      $.on(this.$svg, "mousedown", ".today-highlight", (e, element) => {
      ...
      $.on(this.$svg, "mousemove", (e) => {
        if (is_dragging_current_date) {
          const dx = (e.offsetX - x_on_start) / this.options.column_width;
          if (this.view_is("Day"))
            this.set_current_date(
              date_utils.add(currentdate_on_start, dx, "day")
            );
          if (this.view_is("Half Day"))
            this.set_current_date(
              date_utils.add(currentdate_on_start, dx * 0.5, "day")
            );
          ...
          if (this.view_is("Week"))
            this.set_current_date(
              date_utils.add(currentdate_on_start, dx * 7, "day")
            );
          else if (this.view_is("Month")) {
            this.set_current_date(
              date_utils.add(currentdate_on_start, dx * 30, "day")
            );
          }
          e.preventDefault();
          return;
